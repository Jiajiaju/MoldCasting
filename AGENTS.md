...
- This repository contains an Unreal Engine 5.8 C++ project named `MoldCasting`.
...
- For source changes, compile the `MoldCastingEditor` target for Win64 Development when a compatible Unreal Engine 5.8 build is available.
- If the engine location is unavailable, state that limitation and still run relevant static checks and inspect the diff.
